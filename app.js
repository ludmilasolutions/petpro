// app.js - Código JavaScript separado para Tu Mascota Online
...
function setupEventListeners() {
    ...
    // Modal de mascotas
    document.getElementById('pet-modal-close').addEventListener('click', () => petModal.classList.remove('active'));
    document.getElementById('pet-modal-cancel').addEventListener('click', () => petModal.classList.remove('active'));
    document.getElementById('pet-modal-save').addEventListener('click', handleSavePet);
    
    // Modal de autorización veterinaria
    document.getElementById('auth-vet-modal-close').addEventListener('click', () => authVetModal.classList.remove('active'));
    
    // Modal de historial médico
    document.getElementById('medical-record-modal-close').addEventListener('click', () => medicalRecordModal.classList.remove('active'));
    document.getElementById('medical-record-modal-cancel').addEventListener('click', () => medicalRecordModal.classList.remove('active'));
    document.getElementById('medical-record-modal-save').addEventListener('click', handleSaveMedicalRecord);
    
    // Modal de turnos
    document.getElementById('appointment-modal-close').addEventListener('click', () => appointmentModal.classList.remove('active'));
    document.getElementById('appointment-modal-cancel').addEventListener('click', () => appointmentModal.classList.remove('active'));
    document.getElementById('appointment-modal-save').addEventListener('click', handleSaveAppointment);
    
    // Modal de detalles de veterinaria
    document.getElementById('vet-details-modal-close').addEventListener('click', () => vetDetailsModal.classList.remove('active'));
    ...
    window.addEventListener('click', (e) => {
        ...
        if (e.target === authVetModal) authVetModal.classList.remove('active');
        if (e.target === medicalRecordModal) medicalRecordModal.classList.remove('active');
        if (e.target === appointmentModal) appointmentModal.classList.remove('active');
        if (e.target === vetDetailsModal) vetDetailsModal.classList.remove('active');
        ...
        html += `
            <div class="cards-grid">
                ...
                <div class="card">
                    ...
                    <p>${appointments.filter(a => a.status === 'scheduled' || a.status === 'confirmed').length} turno${appointments.filter(a => a.status === 'scheduled' || a.status === 'confirmed').length !== 1 ? 's' : ''} pendiente${appointments.filter(a => a.status === 'scheduled' || a.status === 'confirmed').length !== 1 ? 's' : ''}</p>
        ...
        html += `
            ...
            <div class="cards-grid">
                ...
                <div class="card">
                    ...
                    <p>${vetAppointments.filter(a => isToday(new Date(a.dateTime))).length} turno${vetAppointments.filter(a => isToday(new Date(a.dateTime))).length !== 1 ? 's' : ''} programado${vetAppointments.filter(a => isToday(new Date(a.dateTime))).length !== 1 ? 's' : ''}</p>
        ...
        medicalRecords.forEach(record => {
            if (!recordsByPet[record.petId]) {
                recordsByPet[record.petId] = {
                    pet: pets.find(p => p.id === record.petId),
                    records: []
                };
            }
...
async function loadVetSettings() {
    ...
    if (enableAppointments) {
        enableAppointments.addEventListener('change', function() {
            document.getElementById('appointments-settings').style.display = this.checked ? 'block' : 'none';
        });
    }
}
...
async function loadAllVets() {
    try {
        ...
        for (const doc of vetsSnapshot.docs) {
            ...
            vetData.uid = doc.id;
            allVets.push(vetData);
        }
        
        console.log(`${allVets.length} veterinarias cargadas`);
        renderVetsList(allVets);
        
    } catch (error) {
        console.error('Error al cargar veterinarias:', error);
        document.getElementById('vets-list-container').innerHTML = `
            <div class="empty-state">
                <div class="empty-icon">🏥</div>
...
function renderVetsList(vets) {
    ...
    vets.forEach(vet => {
        ...
        html += `
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">${vet.vetInfo?.name || vet.displayName || 'Veterinaria'}</h3>
                    <span class="status-badge ${vet.vetStatus === 'trial' ? 'status-trial' : 'status-active'}">
                        ${vet.vetStatus === 'trial' ? 'Prueba' : 'Activa'}
                    </span>
                </div>
                
                <p><strong>📞 ${vet.vetInfo?.phone || 'Sin teléfono'}</strong></p>
                <p>${vet.vetInfo?.address || 'Sin dirección'}</p>
                ${vet.vetInfo?.city ? `<p>${vet.vetInfo.city}</p>` : ''}
                
                ${vet.vetInfo?.specialties ? `
                    <div style="margin-top: 0.5rem;">
                        <strong>Especialidades:</strong>
                        <p>${vet.vetInfo.specialties}</p>
                    </div>
                ` : ''}
                
                <div style="margin-top: 1rem;">
                    <p><strong>Turnos:</strong> ${vet.vetConfig?.appointmentsEnabled ? '✅ Disponibles' : '❌ No disponibles'}</p>
                </div>
...
async function showAuthVetModal() {
    ...
    if (unauthorizedVets.length === 0) {
        html = `
            <div class="empty-state" style="padding: 2rem;">
                ...
                <p>Todas las veterinarias están autorizadas o no hay veterinarias registradas.</p>
            </div>
        `;
    } else {
        html = `
        ...
        unauthorizedVets.forEach(vet => {
            html += `
                <div class="card" style="margin: 0;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <h4 style="margin-bottom: 0.3rem;">${vet.vetInfo?.name || vet.displayName || 'Veterinaria'}</h4>
                            <p style="margin-bottom: 0.3rem; color: var(--gray); font-size: 0.9rem;">${vet.vetInfo?.address || 'Sin dirección'}</p>
                            <p style="margin-bottom: 0.3rem; color: var(--gray); font-size: 0.9rem;">📞 ${vet.vetInfo?.phone || 'Sin teléfono'}</p>
                        </div>
                        <button class="btn btn-primary" onclick="authorizeVet('${vet.uid}')">Autorizar</button>
                    </div>
                </div>
            `;
        });
    ...
    document.getElementById('vet-search-results').innerHTML = html;
...
async function showNewMedicalRecordModal(petId = null) {
    ...
    if (userData.userType === 'vet') {
        ...
        // Para dueño: mostrar todas sus mascotas
        pets.forEach(pet => {
...
async function searchPets() {
    ...
    // TODO: Implementar búsqueda real en Firestore
    ...
    document.getElementById('search-results').innerHTML = `
        <div class="empty-state">
            ...
            <p><small>Podrás buscar mascotas por nombre, dueño o teléfono.</small></p>
        </div>
    `;
}
...
async function showPetQR(petId) {
    ...
    let html = `
        <div style="text-align: center; padding: 2rem;">
            ...
            <div class="qr-code" style="margin: 0 auto 2rem;">
                <!-- En una implementación real, aquí iría un código QR generado -->
                <div style="font-size: 5rem;">🐾</div>
            </div>
            
            <p style="color: var(--gray); font-size: 0.9rem;">ID: ${petId}</p>
            <p style="color: var(--gray); font-size: 0.9rem;">URL: ${qrUrl}</p>
...
window.viewAppointmentDetails = (appointmentId) => {
    alert(`Ver detalles de turno ${appointmentId} - En desarrollo`);
};

window.downloadQR = () => {
    alert('Descargar QR - En desarrollo');
};

window.sharePet = (petId) => {
    alert(`Compartir mascota ${petId} - En desarrollo`);
};
